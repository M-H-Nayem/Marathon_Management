...
const MyApplyList = () => {
  ...
  useEffect(() => {
    axios(`http://localhost:5000/my-marathons-apply/${user.email}`)
      .then((res) => {
  ...
  let handleDeleteMarathonApply = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`http://localhost:5000/marathons-apply/${id}`, {
          method: "DELETE",
        })
          .then((res) => res.json())
          .then((data) => {
  ...
  useEffect(() => {
    if (selectId) {
      ...
      axios(`http://localhost:5000/marathons-apply/${selectId}`)
        .then((res) => {
  ...
  let handleUpdateMarathonpply = (e, id) => {
    e.preventDefault();
    let form = e.target;
    ...
    axios
      .put(`http://localhost:5000/marathons-apply/${id}`, formDateObj)
      .then((res) => {
        console.log(res.data);
                        ...
                        {isOpen && (
                          <div className="fixed inset-0  bg-opacity-10 flex h-screen py-50 justify-center items-center z-50">
                            <div className="bg-gray-700 rounded-3xl shadow-lg w-180  relative">
                              ...
                              <form
                                onSubmit={(e) =>
                                  handleUpdateMarathonpply(e, selectedApplyData._id)
                                }
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <input
                                    type="text"
                                    name="first_name"
                                    defaultValue={marathon.first_name}
                                    className="w-full p-2 border rounded bg-gray-800"
                                    required
                                  />
                                </div>
                                <div>
                                  ...
                                  <input
                                    type="text"
                                    name="last_name"
                                    defaultValue={marathon.last_name}
                                    className="w-full p-2 border rounded bg-gray-800"
                                    required
                                  />
                                </div>
                                ...
                                <div>
                                  ...
                                  <textarea
                                    ...
                                    defaultValue={marathon.additional_info}
                                    rows={4}
                                    className="w-full p-2 border rounded"
                                    required
                                  ></textarea>
